...
SELECT p.id_producto, c.nombre_categoria, v.nombre_vendedor, p.nombre_producto, p.estado, p.precio, p.descuento, p.precio_final
FROM PRODUCTO p
...
SELECT v.nombre_vendedor, p.id_producto, p.nombre_producto, p.estado, p.precio, p.descuento, p.precio_final
FROM PRODUCTO p
JOIN VENDEDOR v ON p.id_vendedor = v.id_vendedor
