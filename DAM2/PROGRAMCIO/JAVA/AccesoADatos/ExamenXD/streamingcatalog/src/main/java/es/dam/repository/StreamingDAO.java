...
public class StreamingDAO {
    ...
    public Long crearCritica(String autor, Integer puntuacion, String comentario, Long peliculaId) {
        ...
        try {
            ...
            id = c.getId();
            tx.commit();
            System.out.println("✅ Critica creada: " + c);
        } catch (Exception ex) {
            if (tx != null) tx.rollback();
    ...
    public boolean actualizarAnioPelicula(Long peliculaId, Integer nuevoAnio) {
        ...
        try {
            ...
            if (p != null) {
                ...
                tx.commit();
                System.out.println("✅ Año de pelicula actualizado: " + p.getTitulo());
    ...
    public boolean eliminarCritica(Long criticaId) {
        ...
        try {
            ...
            if (c != null) {
                session.remove(c);
                tx.commit();
                System.out.println("🗑️ Critica eliminada: id=" + criticaId);
                exito = true;
            } else {
                System.out.println("❌ Critica no encontrada: id=" + criticaId);
            }
        } catch (Exception ex) {
            if (tx != null) tx.rollback();
    ...
    public List<Pelicula> findByAnioMayorQue(int anio) {
        ...
        try {
            ...
            System.out.println("🔍 Peliculas encontradas con año > " + anio + ": " + results.size());
        } catch (Exception ex) {
